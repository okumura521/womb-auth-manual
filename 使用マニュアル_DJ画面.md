...
flowchart TD
  ...
  SelectEvent --> Assign[料金タイプと人数を割当て]
  Assign --> Copy[招待URLを発行・コピー・共有]
...
1. DJダッシュボードでイベントを選択
2. 「割当て＆招待URL発行」タブにて、ゲスト代表者名と人数と料金タイプ（フリー／有料など）を割り当て
3. 「招待URL発行」を押下
4. 「発行状況＆入場記録」タブの発行状況にある発行された招待URLをコピーしてゲスト代表者へ渡す

### 発行済み招待・入場記録を確認する

1. 「発行状況＆入場記録」タブを選択
2. 「発行状況」や「入場記録」のセクションを表示
...
2. **ステータス** … 招待の状態（未確認・確認済み・入場済等）。子ゲストがある場合は、親と同じ行に表示される
3. **総人数** … その招待の総人数（名）
4. **プラン別割当** … 料金タイプごとの人数（例: フリー×2名、2000円×1名）
5. **入場人数** … すでに入場した人数。親・子の内訳が表示される
...
9. **ゲスト代表者名** … 招待の親の名前
10. **分割入場代表者名** … 子で入場した場合の代表者名（親の場合は「-」等）
